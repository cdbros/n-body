...
SimView::SimView() { connect(this, &QQuickItem::windowChanged, this, &SimView::handleWindowChanged); }

void SimView::sync() {
    if (!m_renderer) {
        m_renderer = std::make_unique<Renderer>();
        connect(window(), &QQuickWindow::afterRendering, m_renderer.get(), &Renderer::render, Qt::DirectConnection);
    }
    updateViewportSize();
